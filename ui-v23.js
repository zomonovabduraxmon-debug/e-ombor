(function(root, factory){
  ...
  function buildV23Metrics(source){
    ...
    for(const permit of permits){
      for(const item of (Array.isArray(permit.items) ? permit.items : [])){
        items += 1;
        allowedQty += Number(item.qty)||0;
        allowedWeight += Number(item.weight)||0;
        const material = String(item.material || '').trim() || 'Other';
        const existing = materialMap.get(material) || {name:material,count:0,weight:0};
        existing.count += 1;
        existing.weight += Number(item.weight)||0;
        materialMap.set(material,existing);
      }
    }

    for(const shipment of shipments){
      for(const line of (Array.isArray(shipment.lines) ? shipment.lines : [])){
        shippedQty += Number(line.qty)||0;
        shippedWeight += Number(line.weight)||0;
      }
    }
    ...
    return {
      ...
      shippedQty,
      shippedWeight,
      remainingQty:allowedQty-shippedQty,
      remainingWeight:allowedWeight-shippedWeight,
      materials
    };
  }
  ...
  if(typeof window !== 'undefined' && typeof document !== 'undefined'){
    ...
    function decorateV22PermitModal(){
      const modal = document.querySelector('.modal-backdrop .modal');
      if(!modal) return;

      if(modal.classList.contains('v20-permit-detail-modal')){
        modal.classList.add('v22-permit-detail');
        return;
      }

      if(!modal.classList.contains('v20-permit-modal')) return;
      ...
      const isEdit = text.includes('изменить') || text.includes('tahrir') || text.includes('edit') || text.includes('düzen');
      modal.classList.toggle('v22-permit-edit',isEdit);
      modal.classList.toggle('v22-permit-create',!isEdit);

      if(!head.querySelector('.v22-modal-title-wrap')){
        const wrap = document.createElement('div');
        wrap.className = 'v22-modal-title-wrap';

        const icon = document.createElement('span');
        icon.className = 'v22-modal-head-icon';
        icon.setAttribute('aria-hidden','true');
        icon.innerHTML = `
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            <path d="M6 3h9l3 3v15H6z"/><path d="M15 3v4h4"/><path d="M9 11h6M9 15h6"/>
          </svg>`;

        const copy = document.createElement('div');
        copy.className = 'v22-modal-head-copy';
        ...
        sub.className = 'v22-modal-subtitle';
        sub.textContent = isEdit
          ? (currentLang==='uz' ? 'Ma’lumotlarni tahrirlang va o‘zgarishlarni saqlang'
            : currentLang==='ru' ? 'Измените данные и сохраните изменения'
            : currentLang==='tr' ? 'Bilgileri düzenleyin ve değişiklikleri kaydedin'
            : 'Edit the details and save your changes')
          : (currentLang==='uz' ? 'Ruxsatnoma ma’lumotlari va pozitsiyalarni kiriting'
            : currentLang==='ru' ? 'Введите данные разрешения и его позиции'
            : currentLang==='tr' ? 'İzin bilgilerini ve kalemleri girin'
            : 'Enter permit details and item positions');
        copy.appendChild(sub);

        if(isEdit){
          const permitInput = modal.querySelector('#fldNumber');
          const badge = document.createElement('span');
          badge.className = 'v22-edit-badge';
          badge.textContent = permitInput?.value || '';
          if(badge.textContent) copy.appendChild(badge);
        }
      }

      const host = modal.querySelector('#itemsHost');
      const section = host?.parentElement;
      if(section && !section.classList.contains('v22-positions-section')){
        section.classList.add('v22-positions-section');

        const count = document.createElement('span');
        count.className = 'v22-position-count';
        const rows = host.querySelectorAll('[data-item-row]').length;
        count.textContent = `${rows}`;
        section.insertBefore(count,host);

        const label = document.createElement('strong');
        label.className = 'v22-positions-label';
        label.textContent = currentLang==='uz' ? 'Pozitsiyalar'
          : currentLang==='ru' ? 'Позиции'
          : currentLang==='tr' ? 'Kalemler'
          : 'Items';
        section.insertBefore(label,count);
      }

      const foot = modal.querySelector('.modal-foot');
      if(foot){
        foot.classList.add('v22-modal-foot');
        let meta = foot.querySelector('.v22-foot-meta');
        if(!meta){
          meta = document.createElement('div');
          meta.className = 'v22-foot-meta';
          foot.insertBefore(meta,foot.firstChild);
        }
        const rows = modal.querySelectorAll('#itemsHost [data-item-row]').length;
        meta.innerHTML = `<span class="v22-foot-dot"></span><strong>${rows}</strong><span>${currentLang==='uz'?'pozitsiya':currentLang==='ru'?'позиций':currentLang==='tr'?'kalem':'items'}</span>`;
      }
    }

    function decorateV22Surface(){
      document.documentElement.classList.add('v22-design-ready');
      document.body.classList.add('v22-app-surface');

      document.querySelector('.topbar')?.classList.add('v22-topbar');
      document.querySelector('.tabs')?.classList.add('v22-tabs');
      document.querySelector('#app')?.classList.add('v22-app');

      document.querySelectorAll('#app .toolbar').forEach(el=>el.classList.add('v22-page-toolbar'));
      document.querySelectorAll('#app .panel').forEach(el=>el.classList.add('v22-panel'));
      document.querySelectorAll('#app table').forEach(el=>el.classList.add('v22-table'));

      const summary = document.querySelector('#app .warehouse-summary');
      if(summary) summary.classList.add('v22-warehouse-summary');

      document.querySelectorAll('#app .kpi-card, #app [class*="kpi"]').forEach(el=>{
        if(el.classList.contains('v12-kpi-delta')) return;
        el.classList.add('v22-kpi-surface');
      });

      decorateV22PermitModal();
    }
    ...
    function v23Lang(){
      const key = String(document.documentElement.dataset.lang || 'uz').toLowerCase();
      return ['uz','ru','en','tr'].includes(key) ? key : 'uz';
    }

    function v23Text(){
      const all = {
        uz:{home:'Bosh sahifa',permits:'Ruxsatnomalar',shipment:'Yangi yuklama',export:'Eksport',reports:'Hisobotlar',settings:'Sozlamalar',logout:'Chiqish',search:'Ruxsatnoma, mijoz yoki mahsulot bo‘yicha qidirish...',homeTitle:'Bosh sahifa',homeSub:'Tizim bo‘yicha umumiy ma’lumotlar',addPermit:'Ruxsatnoma qo‘shish',permitSub:'Ruxsatnomalar va ularning pozitsiyalari reyestri',shipmentSub:'Yuklama yaratish va ro‘yxatdan o‘tkazish',exportSub:'Eksport operatsiyalari va hisobotlar',reportsSub:'Ombor va yuklamalar bo‘yicha ko‘rsatkichlar',permitsKpi:'Ruxsatnomalar',shipmentsKpi:'Yuklamalar',remainingKpi:'Qoldiq',materialsKpi:'Materiallar',shipmentDynamics:'Yuklama dinamikasi',materialShare:'Materiallar bo‘yicha ulush',last7:'So‘nggi 7 yuklama',positions:'Pozitsiyalar',all:'Barchasi'},
        ru:{home:'Главная',permits:'Разрешения',shipment:'Новая отгрузка',export:'Экспорт',reports:'Отчёты',settings:'Настройки',logout:'Выйти',search:'Поиск по разрешению, клиенту или товару...',homeTitle:'Главная',homeSub:'Общая информация по системе',addPermit:'Добавить разрешение',permitSub:'Реестр разрешений и их позиций',shipmentSub:'Создание и регистрация отгрузки',exportSub:'Операции экспорта и отчёты',reportsSub:'Показатели склада и отгрузок',permitsKpi:'Разрешения',shipmentsKpi:'Отгрузки',remainingKpi:'Остаток',materialsKpi:'Материалы',shipmentDynamics:'Динамика отгрузок',materialShare:'Доля по материалам',last7:'Последние 7 отгрузок',positions:'Позиции',all:'Все'},
        en:{home:'Dashboard',permits:'Permits',shipment:'New shipment',export:'Export',reports:'Reports',settings:'Settings',logout:'Sign out',search:'Search permit, client or product...',homeTitle:'Dashboard',homeSub:'System overview and inventory status',addPermit:'Add permit',permitSub:'Registry of permits and their items',shipmentSub:'Create and register a shipment',exportSub:'Export operations and reports',reportsSub:'Warehouse and shipment metrics',permitsKpi:'Permits',shipmentsKpi:'Shipments',remainingKpi:'Remaining',materialsKpi:'Materials',shipmentDynamics:'Shipment dynamics',materialShare:'Share by material',last7:'Last 7 shipments',positions:'Items',all:'All'},
        tr:{home:'Ana sayfa',permits:'İzinler',shipment:'Yeni sevkiyat',export:'Dışa aktar',reports:'Raporlar',settings:'Ayarlar',logout:'Çıkış',search:'İzin, müşteri veya ürün ara...',homeTitle:'Ana sayfa',homeSub:'Sistem geneli ve stok durumu',addPermit:'İzin ekle',permitSub:'İzinler ve kalemleri',shipmentSub:'Sevkiyat oluşturma ve kaydetme',exportSub:'Dışa aktarma ve rapor işlemleri',reportsSub:'Depo ve sevkiyat göstergeleri',permitsKpi:'İzinler',shipmentsKpi:'Sevkiyatlar',remainingKpi:'Kalan',materialsKpi:'Malzemeler',shipmentDynamics:'Sevkiyat dinamiği',materialShare:'Malzeme dağılımı',last7:'Son 7 sevkiyat',positions:'Kalemler',all:'Tümü'}
      };
      return all[v23Lang()] || all.uz;
    }

    function v23Icon(name){
      const paths = {
        home:'<path d="M3 11.5 12 4l9 7.5"/><path d="M5.5 10.5V20h13v-9.5"/><path d="M9.5 20v-6h5v6"/>',
        permits:'<rect x="6" y="4" width="12" height="16" rx="2"/><path d="M9 4.5V3h6v1.5M9 9h6M9 13h6M9 17h4"/>',
      ...
      return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name]||paths.home}</svg>`;
    }

    function activeV23Tab(){
      if(currentTab==='reports') return 'reports';
      return currentTab || document.querySelector('.tab-btn.active')?.dataset.tab || 'dashboard';
    }

    function updateV23Nav(){
      const active = activeV23Tab();
      document.querySelectorAll('.v23-nav-btn').forEach(btn=>{
        const on = btn.dataset.v23Tab===active;
        btn.classList.toggle('active',on);
        btn.setAttribute('aria-current',on?'page':'false');
      });
    }

    function createV23Sidebar(){
      document.querySelectorAll('.v11-sidebar,.v12-sidebar').forEach(el=>el.remove());
      let side = document.querySelector('.v23-sidebar');
      const copy = v23Text();
      if(!side){
        side = document.createElement('aside');
        side.className = 'v23-sidebar';
        side.innerHTML = `
          <div class="v23-sidebar-brand"></div>
          <nav class="v23-sidebar-nav" aria-label="Main navigation"></nav>
          <div class="v23-sidebar-bottom"></div>`;
        document.body.appendChild(side);
      }

      const brandHost = side.querySelector('.v23-sidebar-brand');
      const brand = document.querySelector('.brand');
      if(brand && brand.parentElement!==brandHost) brandHost.appendChild(brand);

      const items = [
        ['dashboard','home',copy.home],['permits','permits',copy.permits],['shipment','shipment',copy.shipment],['export','export',copy.export],['reports','reports',copy.reports]
      ];
    ...
    function createV23TopSearch(){
      const top = document.querySelector('.topbar-inner');
      if(!top) return;
      let shell = top.querySelector('.v23-top-search');
      if(!shell){
        ...
        input.addEventListener('input',()=>{
          if(currentTab==='dashboard'){
            const source = document.getElementById('dashSearch');
            if(source){ source.value=input.value; source.dispatchEvent(new Event('input',{bubbles:true})); }
          }else if(currentTab==='permits'){
            const q=input.value.trim().toLowerCase();
            document.querySelectorAll('.v21-permits-table tbody tr').forEach(row=>row.hidden=!!q && !row.textContent.toLowerCase().includes(q));
          }
        });
        input.addEventListener('keydown',e=>{ if(e.key==='Escape'){ input.value=''; input.dispatchEvent(new Event('input',{bubbles:true})); }});
      }
      const input=shell.querySelector('input');
      ...
      shell.dataset.lastTab=currentTab;
    }

    function v23ShipmentBars(){
      const rows = (Array.isArray(state.shipments)?state.shipments:[]).slice()
    ...
    function v23MaterialDonut(metrics){
      const materials=metrics.materials.filter(x=>x.weight>0);
      if(!materials.length) return {style:'background:#edf3fa',legend:'<div class="v23-chart-empty">—</div>'};
      const total=materials.reduce((s,x)=>s+x.weight,0)||1;
      const palette=['#1668d2','#24a5bd','#46c5a6','#79d2b8','#a6c4eb'];
      let at=0;
      const stops=[];
      const legend=[];
      materials.forEach((m,i)=>{
        const pct=m.weight/total*100;
        stops.push(`${palette[i%palette.length]} ${at}% ${at+pct}%`);
        at+=pct;
        legend.push(`<div><i style="background:${palette[i%palette.length]}"></i><span>${safeHtmlV20(m.name)}</span><strong>${Math.round(pct)}%</strong></div>`);
      });
      return {style:`background:conic-gradient(${stops.join(',')})`,legend:legend.join('')};
    }

    function createV23DashboardAnalytics(){
      const app=document.getElementById('app');
      if(!app || currentTab!=='dashboard') return;
      app.classList.add('v23-dashboard');
      const metrics=buildV23Metrics(state);
      const copy=v23Text();
      const old=app.querySelector('.v23-dashboard-overview');
      const signature=JSON.stringify({
        lang:v23Lang(),
        permits:metrics.permits,
        shipments:metrics.shipments,
        items:metrics.items,
        remainingWeight:metrics.remainingWeight,
        allowedWeight:metrics.allowedWeight,
        materials:metrics.materials.map(x=>[x.name,x.weight]),
        recent:(Array.isArray(state.shipments)?state.shipments:[]).slice(-7).map(s=>[s.id,s.invoiceDate,(Array.isArray(s.lines)?s.lines:[]).length])
      });
      if(old && old.dataset.v23Signature===signature) return;

      const toolbar=app.querySelector('.toolbar');
      const panel=app.querySelector('.panel');
      if(!toolbar || !panel) return;
      const donut=v23MaterialDonut(metrics);
      const overview=document.createElement('section');
      overview.className='v23-dashboard-overview';
      overview.dataset.v23Signature=signature;
      overview.innerHTML=`
        <div class="v23-kpi-grid">
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('permits')}</span><div><small>${copy.permitsKpi}</small><strong>${metrics.permits}</strong><span>${copy.positions}: ${metrics.items}</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('shipment')}</span><div><small>${copy.shipmentsKpi}</small><strong>${metrics.shipments}</strong><span>${fmt(metrics.shippedWeight,0)} kg</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('export')}</span><div><small>${copy.remainingKpi}</small><strong>${fmt(metrics.remainingWeight,0)}</strong><span>kg</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('reports')}</span><div><small>${copy.materialsKpi}</small><strong>${metrics.materials.length}</strong><span>${copy.positions}: ${metrics.items}</span></div></article>
        </div>
        <div class="v23-analytics-grid">
          <article class="v23-chart-card"><header><div><strong>${copy.shipmentDynamics}</strong><span>${copy.last7}</span></div></header><div class="v23-bars">${v23ShipmentBars()}</div></article>
          <article class="v23-chart-card v23-material-card"><header><div><strong>${copy.materialShare}</strong><span>${fmt(metrics.allowedWeight,0)} kg</span></div></header><div class="v23-material-body"><div class="v23-donut" style="${donut.style}"><span>${metrics.items}<small>${copy.positions}</small></span></div><div class="v23-material-legend">${donut.legend}</div></div></article>
        </div>`;
      if(old) old.replaceWith(overview);
      else panel.parentNode.insertBefore(overview,panel);
    }

    function decorateV23Dashboard(){
      const app=document.getElementById('app');
      if(!app || currentTab!=='dashboard') return;
      const copy=v23Text();
      const toolbar=app.querySelector('.toolbar');
      if(toolbar){
        toolbar.classList.add('v23-page-head');
        const h1=toolbar.querySelector('h1'); if(h1 && h1.textContent!==copy.homeTitle) h1.textContent=copy.homeTitle;
        const desc=toolbar.querySelector('.desc'); if(desc && desc.textContent!==copy.homeSub) desc.textContent=copy.homeSub;
        if(!toolbar.querySelector('.v23-head-actions')){
          const actions=document.createElement('div'); actions.className='v23-head-actions';
          const btn=document.createElement('button'); btn.type='button'; btn.className='btn v23-primary-action'; btn.id='btnNewPermit';
          btn.addEventListener('click',()=>openPermitModal(null)); actions.appendChild(btn); toolbar.appendChild(actions);
        }
        const addBtn=toolbar.querySelector('.v23-primary-action');
        if(addBtn && addBtn.textContent!==`+ ${copy.addPermit}`) addBtn.textContent=`+ ${copy.addPermit}`;
      }

      const controls=app.querySelector('.dashboard-controls');
      const panel=app.querySelector('.panel');
      if(controls && panel){
        let tools=app.querySelector('.v23-table-tools');
        if(!tools){ tools=document.createElement('div'); tools.className='v23-table-tools'; panel.parentNode.insertBefore(tools,panel); }
        if(controls.parentElement!==tools) tools.appendChild(controls);
      }
      createV23DashboardAnalytics();
    }

    function decorateV23Permits(){
      const app=document.getElementById('app');
      if(!app || currentTab!=='permits') return;
      app.classList.add('v23-permits');
      const copy=v23Text();
      const toolbar=app.querySelector('.toolbar');
      if(toolbar){
        toolbar.classList.add('v23-page-head');
        const h1=toolbar.querySelector('h1'); if(h1 && h1.textContent!==copy.permits) h1.textContent=copy.permits;
        const desc=toolbar.querySelector('.desc'); if(desc && desc.textContent!==copy.permitSub) desc.textContent=copy.permitSub;
      }
      const panel=app.querySelector('.panel');
      if(panel){
        const count=panel.querySelectorAll('tbody tr').length;
        const sig=`${v23Lang()}:${count}`;
        let tools=app.querySelector('.v23-permits-tools');
        if(!tools){
          tools=document.createElement('div'); tools.className='v23-permits-tools';
          panel.parentNode.insertBefore(tools,panel);
        }
        if(tools.dataset.v23Signature!==sig){
          tools.dataset.v23Signature=sig;
          tools.innerHTML=`<div><strong>${copy.permits}</strong><span>${count} ${copy.positions.toLowerCase()}</span></div><div class="v23-tool-chip">${copy.all}</div>`;
        }
      }
    }

    function decorateV23Shipment(){
      const app=document.getElementById('app');
      if(!app || currentTab!=='shipment') return;
      app.classList.add('v23-shipment');
      const copy=v23Text();
      const toolbar=app.querySelector('.toolbar');
      if(toolbar){ toolbar.classList.add('v23-page-head'); const d=toolbar.querySelector('.desc'); if(d && d.textContent!==copy.shipmentSub) d.textContent=copy.shipmentSub; }
    }

    function decorateV23Export(){
      const app=document.getElementById('app');
      if(!app || currentTab!=='export') return;
      app.classList.add('v23-export');
      const copy=v23Text();
      const toolbar=app.querySelector('.toolbar');
      if(toolbar){ toolbar.classList.add('v23-page-head'); const d=toolbar.querySelector('.desc'); if(d && d.textContent!==copy.exportSub) d.textContent=copy.exportSub; }
    }

    function renderV23Reports(){
      const app=document.getElementById('app');
      if(!app) return;
      const copy=v23Text();
      const m=buildV23Metrics(state);
      const donut=v23MaterialDonut(m);
      app.innerHTML=`
        <div class="toolbar v23-page-head"><div><h1>${copy.reports}</h1><div class="desc">${copy.reportsSub}</div></div></div>
        <div class="v23-kpi-grid">
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('permits')}</span><div><small>${copy.permitsKpi}</small><strong>${m.permits}</strong><span>${m.items} ${copy.positions.toLowerCase()}</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('shipment')}</span><div><small>${copy.shipmentsKpi}</small><strong>${m.shipments}</strong><span>${fmt(m.shippedWeight,0)} kg</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('export')}</span><div><small>${copy.remainingKpi}</small><strong>${fmt(m.remainingWeight,0)}</strong><span>kg</span></div></article>
          <article class="v23-kpi"><span class="v23-kpi-icon">${v23Icon('reports')}</span><div><small>${copy.materialsKpi}</small><strong>${m.materials.length}</strong><span>${fmt(m.allowedWeight,0)} kg</span></div></article>
        </div>
        <div class="v23-analytics-grid v23-reports-grid">
          <article class="v23-chart-card"><header><div><strong>${copy.shipmentDynamics}</strong><span>${copy.last7}</span></div></header><div class="v23-bars">${v23ShipmentBars()}</div></article>
          <article class="v23-chart-card v23-material-card"><header><div><strong>${copy.materialShare}</strong><span>${fmt(m.allowedWeight,0)} kg</span></div></header><div class="v23-material-body"><div class="v23-donut" style="${donut.style}"><span>${m.items}<small>${copy.positions}</small></span></div><div class="v23-material-legend">${donut.legend}</div></div></article>
        </div>`;
    }

    function decorateV23Surface(){
      document.documentElement.classList.add('v23-shell-ready');
      createV23Sidebar();
      createV23TopSearch();
      const app=document.getElementById('app');
      if(app) app.classList.remove('v23-dashboard','v23-permits','v23-shipment','v23-export');
      if(currentTab==='dashboard') decorateV23Dashboard();
      else if(currentTab==='permits') decorateV23Permits();
      else if(currentTab==='shipment') decorateV23Shipment();
      else if(currentTab==='export') decorateV23Export();
      updateV23Nav();
    }

    function installV23Shell(){
      if(document.documentElement.dataset.v23Installed==='1') return;
      document.documentElement.dataset.v23Installed='1';
      decorateV23Surface();
    }
